...
exports.scanUrl = function (args, res, next) {
  ...
  getBufferFromUrl(fileUrl)
    .then(
      (pdfBuffer) => testPDFBuffer(pdfBuffer, maxPages),
      (err) => {
        throw err;
      }
    )
    ...
    .catch((err) => sendAPIResponse(res, "", err));
};
...
exports.scanFile = function (args, res, next) {
  ...
  testPDFBuffer(pdfBuffer, maxPages)
    .then(
      (results) => {
        results.filename = fileName;
        results.id = scanId;
...
const sendAPIResponse = function (res, data, err) {
  ...
  if (err) {
    ...
    if (data.length) {
      //in case there is data to pass along
      errMsg.data = data;
    }
